...
def layer_stats(
    ...
    if model_name is None:
        model_name = model.config._name_or_path.replace("/", "_").replace("6b","6B")

    stats_dir = Path(stats_dir)
    file_extension = f"{model_name}/{ds_name}_stats/{layer_name}_{precision}_{'-'.join(sorted(to_collect))}{size_suffix}.npz".replace("6b","6B")

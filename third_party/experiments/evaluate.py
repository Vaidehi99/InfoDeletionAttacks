...
def get_metrics(model, input_ids, target_ids, k, layers_wb_attack):
    ...
    min_topk_prob = sorted_layer_logits[:,0,-1,-5]
    max_bottomk_prob = sorted_layer_logits[:,0,-1,4]
...
def sweep_experiment_name(args, model_name, alg_name, ds_name, sweep_params):
  ...
  if args.tracing_reversal:
    ...
    obj = ''
  return f'{exp_name}{obj}_n{args.dataset_size_limit}_t{args.datapoints_to_execute}_secondhalf{args.second_half}_analytical_soln{args.analytical_soln}_mp{args.model_parap}_top-{args.k}_attk_lyr_{layers_wb_attack}_cfdef{args.cf_defense}'
    ...
    for record in tqdm(ds):
        ...
        if rewrite_this_point:
            ...
            with torch.no_grad(): 
                ...
                top_k, bottom_k, top_1, bottom_1, target_prob = get_metrics(edited_model, input_ids, target_ids, args.k, layers_wb_attack)
                # print(type(top_k))
                # print(torch.any(top_k==1.0))
                # # print(np.any((top_k==1.0),-1))
                # print(bottom_k.shape)
                # print(top_1.shape)
                # print(bottom_1.shape)
                # print(target_prob)
                # exit()
                # print(torch.any(top_k==1.0).item())
                # print(torch.any(bottom_k==1.0).item())
                # print(torch.any(torch.tensor([torch.any(top_k==1.0), torch.any(bottom_k==1.0)])).item())
                # exit()
                metrics = {
                    "case_id": case_id,
                ...
                '''
                #pre-edit
                # isin_top5_sum_preedit += isin_top5[:,-1].astype(int)
                # print(isin_top5_sum)
                # target_probs_sum_preedit += probs[:,-1]
                '''
            # print(metrics)
            # exit()
            # state_a = model.state_dict()
            # state_b = edited_model.state_dict()
            # for key in state_a:
            #     if key in state_b:
            #         assert(torch.allclose(state_a[key], state_b[key]))
            #     else:
            #         assert(False)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_name",
        choices=["huggyllama/llama-7b", "gpt2-medium", "gpt2-large", "gpt2-xl", "EleutherAI/gpt-j-6B", "gpt-j-6B"],
        default="gpt2-xl",
        help="Model to edit.",
        required=True,
    )
    ...
    if 'gpt2' in model_name:
        ...
        num_layers = 48
    if '6b' in model_name or '6B' in model_name:
    ...
    metrics = ['actual_retain_rate', 'actual_retain_rate_neighborhood', 'delta_accuracy', 'delta_accuracy_neighborhood', 'retain_rate_neighborhood_pre', 'retain_rate_pre', 'retain_rate_neighborhood', 'retain_rate', 'post_rewrite_success', 'rewrite_prob_diff', 'rewrite_post_prob', 'rewrite_score', 'post_paraphrase_success', 'paraphrase_prob_diff', 'paraphrase_post_prob', 'paraphrase_score', 'post_neighborhood_success', 'neighborhood_prob_diff', 'neighborhood_score', 'essence_ppl_diff', 'topk_metric_agg', 'bottomk_metric_agg', 'topk_or_bottomk_metric_agg', 'top1_metric_agg', 'bottom1_metric_agg', 'top1_or_bottom1_metric_agg', 'target_prob_agg']
    ...
    with open('../results/results_wb_copy.csv', 'a', newline='') as f_object:
